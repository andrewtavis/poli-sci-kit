...
def alloc_to_share_ratio(share, 
                         total_shares, 
                         allocation, 
                         total_alloc):
    """
...
def sqr_alloc_to_share_error(share, 
                             total_shares, 
                             allocation, 
                             total_alloc):
    """
    ...
    """
    asr = alloc_to_share_ratio(share=share, 
...
def total_alloc_to_share_error(shares, 
                               allocations, 
                               proportional=True):
    """
    ...
    """
    assert len(shares) == len(allocations), 'The total different shares of a population or vote must equal that of the allocations.'
    
    sum_share = sum(shares)
    sum_allocations = sum(allocations)

    sqr_asr_errors = [sqr_alloc_to_share_error(share=shares[i], 
                                               total_shares=sum_share, 
                                               allocation=allocations[i], 
                                               total_alloc=sum_allocations) for i in range(len(shares))]

    if proportional:
        proportional_errors = [shares[i] / sum_share * sqr_asr_errors[i] for i in range(len(shares))]
...
def sqr_rep_weight_error(share, 
                         total_shares, 
                         allocation, 
                         total_alloc):
    """
...
def total_rep_weight_error(shares, 
                           allocations, 
                           proportional=True):
    """
    ...
    sqr_rw_errors = [sqr_rep_weight_error(share=shares[i], 
                                          total_shares=sum_share, 
                                          allocation=allocations[i], 
                                          total_alloc=sum_allocations) for i in range(len(shares))]

    if proportional:
        proportional_errors = [shares[i] / sum_share * sqr_rw_errors[i] for i in range(len(shares))]
...
def div_index(shares, 
              q=None, 
              mertric_type='Shannon'):
    """
    ...
    if mertric_type == 'Shannon':
        ...
        assert q, "The order of diversity 'q' argument must be used with Renyi diversity calculations"
        ...
        assert q, "The order of diversity 'q' argument must be used with Effective diversity calculations"
        if q == 1:
        ...
        ValueError(f"{mertric_type} is not a valid value for the 'mertric_type' agrument.")

    return div_index


def effective_number_of_groups(shares, 
                               mertric_type='Laakso-Taagepera'):
    """
    ...
    if mertric_type == 'Laakso-Taagepera':
        ...
        num_groups = sum([share / (share + max_share ** 2 - share ** 2) for share in norm_shares])
...
def dispr_index(shares, 
    ...
    """
    assert len(shares) == len(allocations), "The number of different shares must equal the number of different allocations."
    
    available_metrics = ['Gallagher', 'Loosemore–Hanby', 'Rose', 'Rae', 'Sainte-Laguë', 'd’Hondt', 'Cox-Shugart', 'Gini']
    assert mertric_type in available_metrics, "{} is not a valid value for the 'mertric_type' agrument. Please choose from the following options: " + ', '.join(available_metrics) + '.'

    norm_shares = normalize(vals=shares)
    norm_allocations = normalize(vals=allocations)

    if mertric_type == 'Gallagher':
        dispr_index = sqrt(1.0 / 2) * sqrt(sum([(share - allocation) ** 2 for share, allocation in zip(norm_shares, norm_allocations)]))

    elif mertric_type == 'Loosemore-Hanby':
        dispr_index = 1.0 / 2 * sum([abs(share - allocation) for share, allocation in zip(norm_shares, norm_allocations)])

    elif mertric_type == 'Rose':
         dispr_index = 100 - dispr_index(shares=shares, 
                                         allocations=allocations, 
                                         mertric_type='Loosemore-Hanby')

    elif mertric_type == 'Rae':
        dispr_index = 1.0 / len(norm_shares) * sum([abs(share - allocation) for share, allocation in zip(norm_shares, norm_allocations)])

    elif mertric_type in ['Sainte-Laguë', 'Sainte-Lague']:
        dispr_index = sum(1.0 / share * (share - allocation) ** 2 for share, allocation in zip(norm_shares, norm_allocations))

    elif mertric_type in ['dHondt', 'dhondt', "d’Hondt", "d’hondt"]:
        dispr_index = max([1.0 * allocation / share for share, allocation in zip(norm_shares, norm_allocations)])
