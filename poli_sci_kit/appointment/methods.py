...
def largest_remainder(quota_style='Hare', 
                      shares=None, 
                      total_alloc=None, 
    ...
    """
    assert alloc_threshold == None or min_alloc == None, \
    ...
    if alloc_threshold:
        passed_threshold = [True if 1.0 * s / sum(shares) > alloc_threshold else False for s in shares]
        shares = [shares[i] if passed_threshold[i] == True else 0 for i in range(len(shares))]

    original_remainders = None
    if min_alloc != None and min_alloc > 0:
        assert min_alloc * len(shares) <= total_alloc, "The sum of the minimum seats to be allocated cannot be more than the seats to be allocated."
        ...
        seat_quota = get_quota(quota_style=quota_style, shares=shares, total_alloc=total_alloc)
        original_remainders, original_allocations = zip(*[modf(1.0 * s / seat_quota) for s in shares])
        ...
        original_with_baseline = [original_allocations[i] if original_allocations[i] >= baseline_allocations[i] else baseline_allocations[i] \
    ...
    if min_alloc != None and min_alloc > 0: 
        ...
        if original_with_baseline != baseline_allocations: # we have extra allocations already
            allocations = [0] * len(shares) # don't allocate any more seats based on the quota 
    ...
    # Tie break conditions
        ...
        if tie_break == 'majority':
            sorted_by_results = [i[0] for i in sorted(enumerate(shares), key=itemgetter(1)) \
                                    if i[0] in equal_to_last_assigned][::-1]
            equal_to_highest = [i for i in sorted_by_results \
        ...
        if tie_break == 'random':
            ...
            ValueError(f"A tie break is required for the last seat(s), and an invalid argument '{tie_break}' has been passed. Please choose from 'majority' or 'random'.")

    if min_alloc:
        allocations = [allocations[i] + original_with_baseline[i] for i in range(len(allocations))]

    if majority_bonus:
        # If a single majority group does not receive at least 50%, then they are given it, and assignment is redone for the rest
        if not allocations[shares.index(max(shares))] >= int(ceil(total_alloc / 2)) and len([s for s in shares if s == max(shares)]) == 1:
            non_majority_shares = [s for s in shares if s != max(shares)]
            reduced_seats = total_alloc - int(ceil(total_alloc / 2))
            non_majority_allocations = largest_remainder(quota_style=quota_style,
            ...
            non_majority_allocations[shares.index(max(shares)) : shares.index(max(shares))] = [int(ceil(total_alloc / 2))]
            allocations = non_majority_allocations

    return allocations


def highest_average(averaging_style='Jefferson', 
                    shares=None, 
                    total_alloc=None, 
                    alloc_threshold=None, 
                    min_alloc=None, 
    ...
    assert alloc_threshold == None or averaging_style != 'Huntington-Hill', \
    ...
    if averaging_style == 'Huntington-Hill' and (min_alloc == None or min_alloc == 0):
        print("A minimum allocation is required in the denominator of Huntington-Hill calculations.")
        print("A minimum allocation of 1 will be applied.")
        assert len(shares) <= total_alloc, "There must be at least one seat per group when using the Huntington-Hill method."
        min_alloc = 1

    if alloc_threshold:
        passed_threshold = [True if 1.0 * i / sum(shares) > alloc_threshold else False for i in shares]
        shares = [shares[i] if passed_threshold[i] == True else 0 for i in range(len(shares))]
    ...
    while total_alloc > 0:
        ...
        if averaging_style == 'Jefferson':
            if modifier:
                quotients = [1.0 * s / (allocations[i] + 1) if allocations[i] > 1 else 1.0 * s / modifier \
                    for i, s in enumerate(shares)]

            else:
                quotients = [1.0 * s / (allocations[i] + 1) for i, s in enumerate(shares)]

        elif averaging_style == 'Webster':
            if modifier:
                quotients = [1.0 * s / ((2 * allocations[i]) + 1) if allocations[i] > 1 else 1.0 * s / modifier \
                    for i, s in enumerate(shares)]

            else:
                quotients = [1.0 * s / ((2 * allocations[i]) + 1) for i, s in enumerate(shares)]

        elif averaging_style == 'Huntington-Hill':
            if modifier:
                quotients = [1.0 * s / sqrt(allocations[i] * (allocations[i] + 1)) if allocations[i] > 1 else 1.0 * s / modifier \
                    for i, s in enumerate(shares)]

            else:
                quotients = [1.0 * s / sqrt(allocations[i] * (allocations[i] + 1)) for i, s in enumerate(shares)]

        else:
            print(f"'{averaging_style}' is not a supported highest average method. Please choose from 'Jefferson', 'Webster', or 'Huntington-Hill'")
            print("Naming conventions for methods differ across regions, with United States naming conventions used in social-sci-kit.")
            print("""US assignment method name conversions:
        ...
        # Tie break conditions
            ...
            if tie_break == 'majority':
                sorted_by_results = [i[0] for i in sorted(enumerate(shares), key=itemgetter(1)) \
                                     if i[0] in max_quotient_indexes][::-1]
                equal_to_highest = [i for i in sorted_by_results if shares[i] == shares[sorted_by_results[0]]]
    ...
    if majority_bonus:
        ...
        if not allocations[shares.index(max(shares))] >= int(ceil(total_alloc / 2)) and len([s for s in shares if s == max(shares)]) == 1:
            non_majority_shares = [s for s in shares if s != max(shares)]
            reduced_seats = total_alloc - int(ceil(total_alloc / 2))
            non_majority_allocations = highest_average(shares=non_majority_shares, total_alloc=reduced_seats, 
                                                       alloc_threshold=alloc_threshold, min_alloc=min_alloc, 
