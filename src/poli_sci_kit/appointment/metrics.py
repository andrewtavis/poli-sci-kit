...
def ideal_share(
    share: int | float, total_shares: int | float, total_allocation: int
) -> float:
    """
    ...
    """
    return 1.0 * share / total_shares * total_allocation


def alloc_to_share_ratio(
    share: int, total_shares: int, allocation: int, total_allocation: int
) -> float:
    """
    ...
    """
    return 1.0 * (allocation / total_allocation) / (share / total_shares)


def sqr_alloc_to_share_error(
    share: int, total_shares: int, allocation: int, total_allocation: int
) -> float:
    """
...
def total_allocation_to_share_error(
    shares: list, allocations: list, proportional: bool = True
) -> float:
    """
    ...
    sqr_asr_errors = [
        sqr_alloc_to_share_error(
            ...
            total_allocation=sum_allocations,
        )
...
def representative_weight(share: int, allocation: int) -> float:
    """
...
def sqr_representative_weight_error(
    share: int, total_shares: int, allocation: int, total_allocation: int
) -> float:
    """
    ...
    float
        The squared of the error of the allocation to share ratio.
    """
    rw = representative_weight(share=share, allocation=allocation)

    return (rw - total_shares / total_allocation) ** 2


def total_representative_weight_error(
    shares: list, allocations: list, proportional: bool = True
) -> float:
    """
...
def diversity_index(
    ...
    if metric_type == "Shannon":
        ...
        if q == 1:
            index = exp(diversity_index(shares=shares, q=None, metric_type="Shannon"))
...
def effective_number_of_groups(
    shares: list, metric_type: str = "Laakso-Taagepera"
) -> float:
    """
    Calculate the effective number of groups given vote or population distributions.
    ...
    if metric_type == "Laakso-Taagepera":
        ...
        return 1.0 / diversity_index(shares=shares, metric_type="Shannon")

    else:
        raise ValueError(
            f"Invalid metric_type '{metric_type}'. "
            "Expected 'Laakso-Taagepera', 'Golosov', or 'Inverse-Simpson'."
        )


def disproportionality_index(
    shares: list, allocations: list, metric_type: str = "Gallagher"
) -> float:
    """
    Measure the degree to which the actual allocations deviates from the shares, with larger indexes implying greater disproportionality.
    ...
    if metric_type == "Gallagher":
        ...
        index = 100 - disproportionality_index(
